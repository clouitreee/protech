...
| `CostTransparency` | `Table`, `Thead`, `Tbody`, `Tr`, `Th`, `Td` | Chakra UI | `variant="simple"` | `scope` für Header |
| `ContactIntro` | `Heading`, `Text` | Chakra UI | `as="h1"` | H1 first |
| `ContactCards` | `SimpleGrid`, `Card`, `CardBody`, `Icon`, `Heading`, `Link` | Chakra UI | `columns={{ base: 1, md: 3 }}` | Links mit `aria-label` |
| `ContactForm` | `FormControl`, `FormLabel`, `Input`, `Textarea`, `Checkbox`, `Button` | Chakra UI, React Hook Form | `isInvalid` | `aria-describedby` für Fehler |
| `OfficeHours` | `Table`, `Thead`, `Tbody`, `Tr`, `Th`, `Td` | Chakra UI | `variant="simple"` | `scope` für Header |
| `WhatsAppQuickCTA` | `Button` | Chakra UI | `leftIcon={<WhatsAppIcon />}` | `aria-label` für Icon-Button |
| `MapEmbed` | `AspectRatio`, `iframe` | Chakra UI | `ratio={16 / 9}` | `title` für iframe |
